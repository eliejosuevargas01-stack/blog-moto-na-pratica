  ...
  const renderHero = () => (
    <section key="hero" className="relative w-full overflow-hidden" style={{ height: "75vh", minHeight: "420px" }}>
      <Image 
        ...
        unoptimized={heroImage.includes("/uploads/")}
      />
      ...
      <div>
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-3">
            ...
            <h2 style={TEKO} className="text-[28px] font-semibold uppercase tracking-wide">
              {searchQuery ? t.posts.searchTitle.replace("{query}", searchQuery) : t.posts.title}
            </h2>
          </div>
        </div>
        ...
        {posts.length === 0 ? (
          <div className="bg-card border border-border p-12 text-center">
            ...
            >
              {t.posts.clearSearch} <ArrowRight size={14} />
            </Link>
          </div>
        ) : (
          <>
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-1.5 text-[12px] text-muted-foreground">
                        <Clock size={11} /> {featuredPost.readTime} · {featuredPost.createdAt ? new Date(featuredPost.createdAt).toLocaleDateString(currentLang === "en" ? "en-US" : currentLang === "es" ? "es-ES" : "pt-BR", { day: '2-digit', month: 'short', year: 'numeric' }) : (featuredPost.date instanceof Date ? featuredPost.date.toLocaleDateString(currentLang === "en" ? "en-US" : currentLang === "es" ? "es-ES" : "pt-BR", { day: '2-digit', month: 'short', year: 'numeric' }) : String(featuredPost.date))}
                      </span>
                      <span className="text-[13px] font-bold text-primary uppercase tracking-wider flex items-center gap-1">
                        {t.posts.read} <ArrowRight size={13} />
                      </span>
                    </div>
                  </div>
                </Link>
                    ...
                    <Link href={formatPostUrl(post.slug, post.lang)} className="flex flex-col flex-1">
                      ...
                      <div className="p-5 flex flex-col flex-1">
                        ...
                        <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
                          <Clock size={10} /> {post.readTime} · {post.createdAt ? new Date(post.createdAt).toLocaleDateString(currentLang === "en" ? "en-US" : currentLang === "es" ? "es-ES" : "pt-BR", { day: '2-digit', month: 'short', year: 'numeric' }) : (post.date instanceof Date ? post.date.toLocaleDateString(currentLang === "en" ? "en-US" : currentLang === "es" ? "es-ES" : "pt-BR", { day: '2-digit', month: 'short', year: 'numeric' }) : String(post.date))}
                        </div>
                      </div>
                    </Link>
                  </article>
  ...
  const renderBanner = () => (
    <section key="banner" className="relative overflow-hidden border-t border-b border-border" style={{ height: "220px" }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 z-10">
        <span className="text-primary text-[11px] font-bold uppercase tracking-widest mb-2">{t.banner.badge}</span>
        ...
        <p className="text-[#AAAAAA] text-[14px] mt-3">
          {currentLang === "en" || currentLang === "es" ? t.banner.subtitle : homeContent.bannerSubtitle}
        </p>
      </div>
    </section>
  );
